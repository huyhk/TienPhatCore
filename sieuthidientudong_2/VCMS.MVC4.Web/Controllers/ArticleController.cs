    ...
    public class ArticleController : BaseController
    {
        ...
        public ActionResult Detail(int id = 0)
        {
            using (DataContext db = new DataContext())
            {
                ...
                var timecho = db.Articles
                 ...
                 .Where(a => a.ArticleTypeId == 15).OrderBy(a => a.SortOrder);
        ...
        public ActionResult InfinateScrollCategory(int cateId, int pageSize = 20)
        {
            ...
            var lst = db.Articles
                    ...
                    .Where(a => a.Categories.Any(ac => ac.Id == cateId)).OrderBy(a => a.SortOrder).Skip(0).Take(pageSize).ToList();

            if (lst.Count > 0)
            {
                var type = ArticleType.GetById(2, 1);
                str_html = RenderPartialViewToString("AjaxList", lst);
            }
