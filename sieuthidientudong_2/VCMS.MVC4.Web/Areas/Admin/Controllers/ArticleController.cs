    ...
    public class ArticleController : VCMSAdminController
    {
        ...
        public ActionResult List(int id, int page = 1, int pageSize = 10)
        {
            using (DataContext db = new DataContext())
            {
                ...
                ViewBag.ArticleType = db.ArticleTypes.Include(a => a.ArticleTypeDetail).Include(a => a.Categories.Select(c => c.CategoryDetail)).Single(t => t.Id == id);
                ...
                //var articles = Article.GetByType(id, SiteConfig.LanguageId, flag, pageIndex: page, pageSize: pageSize, sortOrder: sortOrder, direction: direction, includeflags: ArticleIncludeFlags.ARTICLE_TYPE, admin: true);
                //int count = Article.CountByType(id, SiteConfig.LanguageId, flag, true);
                ...
                {
                    articles = Article.GetByUser(SiteConfig.CurrentUser.UserId, id, SiteConfig.LanguageId, flag, pageIndex: page, pageSize: pageSize, sortOrder: sortOrder, direction: direction, includeflags: ArticleIncludeFlags.ARTICLE_TYPE | ArticleIncludeFlags.PRICES | ArticleIncludeFlags.PROPERTIES, admin: true);
                    count = Article.CountByUser(SiteConfig.CurrentUser.UserId, id, SiteConfig.LanguageId, flag, true);
                }
        ...
        public ActionResult Search(int id, string keyword, int categoryId = 0, int page = 1, int pageSize = 20)
        {
            using (DataContext db = new DataContext())
            {
                ...
                ViewBag.ItemCount = Article.SearchCount(id, SiteConfig.LanguageId, keyword, categoryId, flag);
        ...
        public ActionResult Create(int id, FormCollection form)
        {
            if (ModelState.IsValid)
            {
                ...
                using (DataContext db = new DataContext())
                {
                    ArticleType type = db.ArticleTypes.Include(ct => ct.CategoryTypes).Include(at => at.Categories).Include(a => a.Properties).Where(t => t.Id == id).Single();
                    ...
                    string url = model.UrlFriendly.Trim('-');
        ...
        public ActionResult Edit(int id, string returnUrl, FormCollection form)
        {
            using (DataContext db = new DataContext())
            {
                ...
                TryUpdateModel(model);
                ArticleType type = db.ArticleTypes.Include(ct => ct.CategoryTypes).Include(at => at.Categories).Include(at => at.Properties).SingleOrDefault(t => t.Id == model.ArticleTypeId);
                ...
                var details = from d in SiteConfig.Languages
                              select new ArticleDetail()
                              {
                                  ...
                                  ArticleName = Request[string.Format("ArticleDetail[{0}].ArticleName", d.Id).Trim()],
        ...
        private ExcelWorksheet CreateHeader(ExcelWorksheet ws1, ArticleType type, Category category)
        {
            ...
            if (type.HasPrice)
            {
                ws1.Cells[5, rowfix].Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                ws1.Column(colfix).Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                ws1.Cells[5, rowfix++].Value = html.Locale("Gía").ToHtmlString();
                ws1.Column(colfix++).Width = 15;
            }
            ...
            ws1.Cells["B2"].Value = html.Locale("Ngày tháng") + ": " + DateTime.Now.ToString();
            ws1.Cells["B3"].Value = html.Locale("User") + ": " + SiteConfig.CurrentUser.DisplayName;

            ws1.Cells["C2"].Value = html.Locale("Loại") + ": " + type.ArticleTypeDetail[SiteConfig.LanguageId].Name;
            //if (category != null)
            //{
